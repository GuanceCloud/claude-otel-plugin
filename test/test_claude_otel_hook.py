...
class ClaudeOtelHookTest(unittest.TestCase):
    ...
    def test_emit_turn_creates_nested_skill_span_for_skill_tool(self):
        class FakeTraceAPI:
            @staticmethod
            def set_span_in_context(span):
                return span
        ...
        class FakeSpan:
            def __init__(self, name, attributes, context=None, start_time=None):
                self.name = name
                self.attributes = attributes
        ...
        class FakeTracer:
            ...
            def start_span(self, name, context=None, start_time=None, attributes=None):
                span = FakeSpan(name, attributes or {}, context=context, start_time=start_time)
                self.spans.append(span)
                return span
        ...
        messages = [
            ...
            {
                "type": "user",
                "timestamp": "2026-06-30T03:38:36.587Z",
                "toolUseResult": {"success": True, "commandName": "dashboard", "durationMs": 25},
                "message": {
                    "content": [
        ...
        by_name = {span.name: span for span in tracer.spans}
        tool_span = by_name["tool:Skill"]
        skill_span = by_name["skill:dashboard"]
        self.assertEqual(tool_span.attributes["skill.name"], "dashboard")
        self.assertEqual(tool_span.attributes["gen_ai.skill.name"], "dashboard")
        self.assertEqual(skill_span.attributes["skill.name"], "dashboard")
    ...
    def test_emit_turn_uses_tool_result_timestamp_when_duration_missing(self):
        class FakeTraceAPI:
            @staticmethod
            def set_span_in_context(span):
                return span
        ...
        messages = [
            {
                "type": "user",
                "timestamp": "2026-06-30T06:14:34.112Z",
                "message": {"content": "生成仪表板"},
            },
            {
                "type": "assistant",
                "timestamp": "2026-06-30T06:14:40.361Z",
                "message": {
                    "id": "msg-1",
                    "model": "claude-test",
                    "content": [
            ...
            {
                "type": "user",
                "timestamp": "2026-06-30T06:14:41.878Z",
                "toolUseResult": {"success": True, "commandName": "dashboard"},
                "message": {
                    "content": [
                        {"type": "tool_result", "tool_use_id": "tool-1", "content": "Launching skill: dashboard"},
        ...
        tool_span = next(span for span in tracer.spans if span.name == "tool:Skill")
        skill_span = next(span for span in tracer.spans if span.name == "skill:dashboard")
    ...
    def test_metric_recorders_use_genai_semantic_tags(self):
        ...
        self.assertEqual(metrics.agent_operation_count.records[0][1]["status"], "completed")
        self.assertNotIn("outcome", metrics.agent_operation_count.records[0][1])
        ...
        self.assertEqual(metrics.agent_operation_duration.records[0][1]["status"], "completed")
        self.assertNotIn("outcome", metrics.agent_operation_duration.records[0][1])
